...
#### For example 
一個關聯表格中的數個屬性間之功能相依性可能是
- 所在地可決定分公司電話區碼 (即: 所在地->區碼)
- 地址所在之區域可決定郵遞區號
- 員工編號可決定員工姓名 (即: 員工編號->姓名)
...
1. 若Primary Key是由多個屬性組成,且某Non-Key Attribute依賴主鍵之全部而非部分時
...
### 部份功能相依
- 若主鍵是由多個屬性組成,而某非鍵值屬性是依賴主鍵之部分屬性時,則稱該非鍵值屬性**部份相依**於主鍵。
    > ![](https://i.imgur.com/waM6a58.png)
    > (身份證,學號)->Z 如果把其中一個PK拿掉則相依性還存在

### 遞移相依
- 若存在一個Non-Key Attributes T,使得 I->T 且 T->J 的功能相依性均成立,則稱屬性 I與屬性J之間具有遞移相依  
...
反身性(Reflexivity):若B是A的子集合,則A->B  
擴張性(Augmentation):若A ->B,則AC ->BC  
**遞移性(Transitivity):若A ->B且B ->C,則A->C**  
自身決定性(Self-determination):A->A
**分解性(Decomposition):若A →BC,則A->B且A->C** 
聯集性(Union):若A->B且A->C,則A->BC
合成性(Composition):若A->B且C->D,則AC->BD  
虛擬遞移性(Pseudo-transitivity): 若A->B且BC->D,則AC->D  
...
> Normalization定義
> : 將一個關聯表所有資訊**分解後**數個新關聯表在**合併**仍可以得到原來未分解時的資訊（Lossless Decomposition）
...
:::danger
表格經由正規化分割得愈細,愈可避免資料重覆及一些更新上的異常現象。
在實務上,分割太細的表格,可能會產生一些問題
- 在查詢資料時,可能會用到許多合併 (Join) 運算,此運算易造成查詢動作非常沒有效率  
- 可能產生有損分解  
...
資料表中的所有紀錄屬性的Values都是Atomic Value,其他所有欄位都相依於PK

> Atomic Value
> : 例如 課程欄位不能存入兩科或者兩科以上的課程名稱

實做方法
> 將`多值屬性`分解成多個`Tuples`以及`複合屬性`拆成`簡單屬性  
> ![](https://i.imgur.com/XciaBot.png)

問題
> 造成資料重複性違反正規化法則,則需要在繼續切割  
...
> 若單一分公司有多筆記錄在檔案中,此分公司若有資料異動,則
必須更動多筆資料。如此不僅浪費空間,而且更新時亦浪費時間,也容易造成資料不一致的情況。

**刪除**記錄時 (刪除資料時,會把過多的資訊刪掉)
> 若某分公司只有一筆交易經歷,若我們要刪除此分公司的交易資
料時,則將連帶刪除該分公司資訊。
...
符合2NF的條件為
1. 符合1NF
2. ==每一個Non Key Attribute皆完全功能相依Primary Key==
...
- 符合2NF
- ==每一個Non Key Attribute皆與PK彼此無遞移相依==

![](https://i.imgur.com/aHJKvGX.png)

可能遇到的問題
> 如果資料表的PK是由多個欄位組成則需要利用BNCF正規化在分割

## Boyce-Codd Normal Form, **BNCF**

- 符合3NF
- 每個Determinant都為Candidate Key
    - **亦即每個資料表PK只能由一個單一欄位所有或者PK不可以相依於任一Non Key Attribute**
    - 不可以有部份相依存在
        > 例如(學號,課號)->成績則(課程)->成績或者(學號)->成績拿掉其中一個PK則相依性消失
...
過度分割表格反而導致資料存取效率下降,所以當發生過度正規化時則需要適時地反正規化  
(e.g. 3NF->2NF, 2NF->1NF)
